...
const PositiveNegativeBarChart = ({ data }) => {
    ...
    return (
        <FlexibleWidthXYPlot height={250}>
            <HorizontalGridLines />
            <YAxis />
            <HorizontalGridLines tickValues={[averages.gt.avg]} style={{ stroke: 'green', strokeWidth: '2px', strokeDasharray: [4, 4] }} />
            <HorizontalGridLines tickValues={[averages.lt.avg]} style={{ stroke: 'red', strokeWidth: '2px', strokeDasharray: [4, 4] }} />
            <VerticalBarSeries data={data.map((v, i) => ({ x: i, y: v }))} colorType="literal" getColor={v => v.y < 0 ? 'red' : 'green'} />
        </FlexibleWidthXYPlot>
    );
}
