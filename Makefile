...
build:
	...
	@echo "window.META = { release: \"$(RELEASE)\" };" > public/meta.js
	CI=false yarn build;
...
sentry-release:
	@if [ -z "$(SENTRY_RELEASE)" ]; then \
		...
		export SENTRY_ORG=athenianco SENTRY_PROJECT=webapp; \
		sentry-cli releases new $(SENTRY_RELEASE); \
		sentry-cli releases files $(SENTRY_RELEASE) upload-sourcemaps --url-prefix=~/static/js --no-rewrite build/static/js; \
